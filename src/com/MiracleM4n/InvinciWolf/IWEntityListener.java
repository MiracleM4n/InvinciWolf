    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if ((attacker) == (plugin.getServer().getPlayer(wolf.getOwner()))) {
            if (attacker instanceof Player) {
            	if ((InvinciWolf.Permissions == null && (((Player)attacker).isOp())) || InvinciWolf.Permissions.has(((Player)attacker), "invinciwolf.own")) {
        ...
        if (plugin.getServer().getPlayer(wolf.getOwner()) == null) {
        	if ((attacker instanceof Player)) {
            	if ((InvinciWolf.Permissions == null && (((Player)attacker).isOp())) || InvinciWolf.Permissions.has(((Player)attacker), "invinciwolf.offline")) {
            		event.setCancelled(false);
        		} else {
            		event.setCancelled(true);
            	}
